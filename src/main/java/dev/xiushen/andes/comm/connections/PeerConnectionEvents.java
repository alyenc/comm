...
public class PeerConnectionEvents {
    ...
    public void subscribeByCode(final int messageCode, final MessageCallback callback) {
        subscribersByCode
                .computeIfAbsent(messageCode, integer -> Subscribers.create())
                .subscribe(callback);
    }
    ...
    public void dispatchMessage(final PeerConnection connection, final String message) {
        final DefaultMessage msg = new DefaultMessage(connection, message);
        messageSubscribers.forEach(s -> s.onMessage(msg));
    }

    public void dispatchMessageByCode(final int code, final PeerConnection connection, final String message) {
        final DefaultMessage msg = new DefaultMessage(connection, message);
