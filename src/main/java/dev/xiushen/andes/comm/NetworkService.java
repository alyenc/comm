...
import dev.xiushen.andes.comm.callback.DisconnectCallback;
import dev.xiushen.andes.comm.callback.MessageCallback;
...
public class NetworkService {
    ...
    public CompletableFuture<Void> stop() {
        if (stopped.compareAndSet(false, true)) {
            ...
            return stop.whenComplete((Void, throwable) -> {
                shutdown.countDown();
            });
        } else {
     ...
     * 断开连接
     */
    public void disconnect(final RemotePeer remotePeer, final DisconnectReason reason) {
        PeerConnection connection = aliveConnections.get(remotePeer.identifier());
    ...
    /**
     * 发送消息
     * @param remotePeer 远程节点RemotePeer
     * @param message 发送的信息
     */
    public void send(final RemotePeer remotePeer, final Message message) {
        try {
            PeerConnection connection = aliveConnections.get(remotePeer.identifier());
            if (Objects.nonNull(connection)) {
    ...
    public void subscribeConnect(final ConnectCallback callback) {
        ...
        communication.subscribeConnect(newConnection -> {
            synchronized (this) {
                ...
                aliveConnections.put(newConnection.remoteIdentifier(), newConnection);
    ...
    public void subscribeDisconnect(final DisconnectCallback callback) {
        ...
        communication.subscribeDisconnect(connection -> {
            synchronized (this) {
                ...
                aliveConnections.remove(connection.remoteIdentifier(), connection);
                deadConnections.put(connection.remoteIdentifier(), connection);
            }
        });
    }
